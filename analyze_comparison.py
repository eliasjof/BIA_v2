...
def load_experiment(exp_dir):
    ...
    df = pd.read_pickle(exp_dir / 'summary.pkl')
    with open(exp_dir / 'paths.pkl', 'rb') as f:
        paths = pickle.load(f)
    obs = {}
    obs_path = exp_dir / 'obstacles.pkl'
    if obs_path.exists():
...
def print_experiment_analysis(df, name):
    has_cv = 'cv' in df.columns and 'feasible' in df.columns
    ...
    print(f'=== AGGREGATED METRICS: {name} ===')
    fea = df[df['feasible']]
    rows = []
...
def main():
    ...
    if not exps:
        ...
        print('Each experiment subdirectory must contain summary.pkl')
        return
